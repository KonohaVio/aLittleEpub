...
from time import strftime,localtime#用于获得今日日期
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.et_title.pack()
        self.et_title.insert(0,"BookName")
...
'''
    ...
    def createDirectoryTree(self):
        ...
        for filename in os.listdir(self.Images_dir):
            os.rename(self.Images_dir+filename,f'{self.Images_dir}{count_rename:0>5d}{filename[-4:]}')
            count_rename += 1
        print(">>>图片文件重命名完毕")
...
'''
        ...
        for img in os.listdir(self.Images_dir):          
            ...
            if newfilename[-3:]=='png':
                imgType = 'png'
            self.manifest_itemrefPart += f'''
\t<item id="x{newfilename}" href="Images/{newfilename}" media-type="image/{imgType}"/>
'''
  ...
  <div style="text-align: center; padding: 0pt; margin: 0pt;">
    <a id="Anchor{newfilename}"></a>
    <img src="../Images/{newfilename}" alt="Images to epub"/>
...
'''
        ...
        #xhtml内容write to文件
        htmlFileName = f'page_content.xhtml'
...
'''    
    ...
    def chooseChapter(self,count:int = 1):
      ...
      <content src="Text/page_content.xhtml#Anchor{chapterIndexImgName}"/>
    </navPoint>
...
#count+1是因为那边是从0开始算的（list的下标
  ...
  <a href="page_content.xhtml#Anchor{chapterIndexImgName}">{chapter}</a>
</div>
'''
...
''' 
    ...
    {addTOC}
        ...
        print(f">>>{epubFileName}制作完成！/n epub保存路径:{outputDir}")
        print("-----------finish-----------")
